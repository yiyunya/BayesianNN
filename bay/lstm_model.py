...
class LSTM_Text(nn.Module):
    ...
    def init_hidden(self):
        ...
        return (Variable(weight.new(num_layers, self.batch_size, self.hidden_size).random_()),Variable(weight.new(num_layers, self.batch_size, self.hidden_size).random_()))

    def forward(self, input):
        encode = self.lookup_table(input)
        lstm_out, hidden = self.lstm(encode.transpose(0, 1), self.hidden)
